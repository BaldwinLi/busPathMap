...
export const loadWeChatSdk = (jsApiList) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    Vue.http.post(`${store.getters.appContextPath}getConfig`, params).then(result => {
...
export const scanWxQRCode = (config) => {
  return new Promise(() => {
    Vue.wechat.scanQRCode({
      needResult: config && config.needResult || 0, // 默认为0，扫描结果由微信处理，1则直接返回扫描结果，
      scanType: config && config.scanType || ["qrCode", "barCode"], // 可以指定扫二维码还是一维码，默认二者都有
